...
var PhoneNumberComponent = (function () {
    ...
    PhoneNumberComponent.prototype.updateValue = function () {
        ...
        if (selectedDialCode != null) {
          this.value = this.value.replace(selectedDialCode, '');
        }
        var x = this.value.replace(/\D/g, '').match(/(\d{0,3})(\d{0,3})(\d{0,4})/);
        const phoneNumber = this.value = !x[2] ? x[1] : '(' + x[1] + ') ' + x[2] + (x[3] ? '-' + x[3] : '');

        if (selectedDialCode != null) {
          this.phoneNumber = selectedDialCode + ' ' + phoneNumber;
        } else {
